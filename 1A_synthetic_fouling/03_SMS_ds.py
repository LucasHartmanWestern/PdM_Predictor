...
from utils.constants import Partition, hour_list, csv_headers_3spots, \
	max_vignette_strength, min_vignette_strength
from utils.helper import get_scenario_paths, gen_dust_variables, \
	make_dust_cloud, get_img_slice_pts, denoise_to_binary, apply_synth_transparent_dust
...
def make_dataset_timelines(root_dir, partition, num_days, num_scenarios, growth_rate, clean_interval):
	data_part_path = os.path.join(root_dir, str(partition.value))
	for scenario_num in tqdm(range(1, num_scenarios + 1), desc='Generating timelines for scenarios'):
		min_daily_growth, max_daily_growth = interpret_growth_rate(growth_rate)
		cleaning_flag = 0  # only used for visual inspection of csv file (not used for this script)
		...
		for day in range(1, num_days + 1):
			if day > 1:
				...
				if day % clean_interval == 0:  # preventative maintenance
					dust_vars_list = [gen_dust_variables(), gen_dust_variables(), gen_dust_variables()]
...
def interpret_growth_rate(avg_rate):
	avg_growth_percent = round(100.0 / avg_rate, 1)
	return 0.0, 2.0 * avg_growth_percent
...
def interpret_timeline_csv(timeline_file_path):
	csv_dicts = []
...
def make_dataset_images(root_dir, partition, dust_im_path, num_scenarios):
	for scenario_num in range(1, num_scenarios + 1):
		...
		csv_dicts = interpret_timeline_csv(paths['csv'])
...
if __name__ == '__main__':
	...
	data_dir = '/Users/nick_1/Bell_5G_Data/SMS_ds'
	sm_data_dir = '/Users/nick_1/Bell_5G_Data/sm_SMS_ds'
	...
	make_dataset_timelines(data_dir, part, num_data_days, num_scs, grow_rate, cleaning_rate)  # make timelines
