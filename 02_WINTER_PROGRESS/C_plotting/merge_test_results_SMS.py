...
if __name__ == "__main__":
    ...
    for trial in range(1, 4):
        for model in MODELS:
            for input_type in INPUT_TYPES:
                csv_path = os.path.join(path_to_results, f'trial_{trial}', f'{model}_{input_type}_{target_type}', 'testing_metrics.csv')
    ...
    df.to_csv(os.path.join(save_folder, 'SMS_binary_results.csv'), index=False)
    ...
    for trial in range(1, 4):
        for model in MODELS:
            for input_type in INPUT_TYPES:
                ...
                current_df = current_df[headers]
                df = pd.concat([df, current_df], ignore_index=True)
    df.to_csv(os.path.join(save_folder, 'SMS_multiclass_results.csv'), index=False)
